...
def main():
    ...
    st.write("Carga un archivo XLS con los codigos, nombres y descripcion de los productos")
    ...
    if uploaded_file:
        ...
        if st.button("Generar descripciones"):
            ...
            results_df = results_df.merge(df[['codigo','Imagenes']], left_on='Codigo', right_on='codigo')
            ...
            print("Format time")
            print(t2-t1)
            # Lista de columnas a formatear
            #columns_to_format = ['Especificaciones', 'Ventajas']
            # Aplicar la formateo en paralelo
            #results_df = parallel_apply_formatting(results_df, columns_to_format, format_list_as_bullets)

            # Lista de columnas a formatear
            #columns_to_format = ['Enlaces', 'Imagenes']
            # Aplicar la formateo en paralelo
            #results_df = parallel_apply_formatting(results_df, columns_to_format, format_list_as_bullets_and_links)

            # Verificar enlaces y dividir el DataFrame
